...
const EditDonationRequest = () => {
  ...
  useEffect(() => {
    if (selectedDistrictId) {
      axiosSecure.get(`/upazillas/${selectedDistrictId}`).then((res) => {
  ...
  useEffect(() => {
    if (id && districts.length > 0 && !isDataLoaded) {
      axiosSecure.get(`/donation-requests/${id}`).then((res) => {
        ...
        if (districtId) {
          axiosSecure.get(`/upazillas/${districtId}`).then((res) => {
            setUpazillas(res.data);
            const upazila = res.data.find((u) => u.name === data.recipientUpazila);
            setValue("recipientUpazila", upazila?.id || "");
  ...
  }, [id, axiosSecure, districts, setValue, isDataLoaded]);
